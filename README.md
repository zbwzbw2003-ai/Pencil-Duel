...
从自己的铅笔附近按住鼠标左键并朝出手方向滑动，松开左键后铅笔滑出。鼠标滑动方向决定铅笔方向，滑动距离与按住时长共同决定力度。有效输入时间最长为 0.3 秒，超时后方向和力度保持在 0.3 秒时的状态。触屏设备可使用单指完成同样操作。每次滑程还会受到纸张纹理方向和随机摩擦影响。

任何一方本回合的**完整划痕**进入对手当前位置 16px 范围即获胜。轨迹碰到纸张边缘会立即停止，不会反弹。
